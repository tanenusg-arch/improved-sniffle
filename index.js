// ===================================================================
// MERGED INDEX.JS – All modules combined (final)
// ===================================================================
...
// ===================================================================
// 1. CONFIGURATION (updated from fik_config.py)
// ===================================================================
...
const ADMIN_CHAT_ID = [8453713398, 8636992436, 6153912689]; // array of admin IDs
...
const CACHE_TTL = 600; // Increased to 10 minutes
const DB_PATH = "bot_database.db";
const PROOF_CHANNEL_ID = -1004442339437;
const REPORT_CHANNEL_ID = PROOF_CHANNEL_ID; // same as proof channel
...
// ===================================================================
// 2. EMOJIS & CUSTOM EMOJI IDs (from emojis.js)
// ===================================================================
...
function parse_telegram_name(name) {
  ...
  if (premiumMatch) {
    ...
    if (amount !== null) {
      if (s.includes('year') || s.includes('yr')) {
        amount *= 12;
      }
...
function format_withdrawal_id(wth_id) {
  ...
  if (str.startsWith("WTH-")) {
    return str;
  }
  if (/^\d+$/.test(str)) {
    return `EX${parseInt(str, 10) + 200}`;
  }
  return str;
}
...
// ===================================================================
// 5. G2BULK API CLIENT (from g2bulk.js) – with increased TTL
// ===================================================================
class G2BulkAPIClient {
  ...
  _set_cached(key, data) {
    this._cache.set(key, {
      data,
      expiry: Date.now() / 1000 + this.cache_ttl
    });
  }
  ...
  async _request(method, endpoint, requires_auth = true, params = null, json_data = null, retries = 3) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        const raw_text = typeof resp.data === 'string' ? resp.data : JSON.stringify(resp.data);
        logger.info(`[G2Bulk] ${method} ${url} -> ${resp.status} | body: ${raw_text.slice(0, 500)}`);

        if (resp.status === 429) {
          const wait = Math.pow(2, attempt) + 1;
          logger.warn(`Rate limited (429) on ${url}. Retrying in ${wait}s...`);
          await new Promise(r => setTimeout(r, wait * 1000));
          continue;
        }

        if (resp.status === 401) {
          return { success: false, message: "Unauthorized API access." };
        }
  ...
  async get_me() {
    return await this._request("GET", "getMe", true);
  }
  ...
  async get_games() {
    ...
    if (res && res.success) {
      this._set_cached(cache_key, res);
    }
    return res;
  }

  async check_player_id(game_code, player_id) {
    return await this._request("POST", "games/checkPlayerId", true, null, {
      game: game_code,
      user_id: player_id
    });
  }
  ...
  async place_order(game_code, catalogue_name, player_id, idempotency_key = null) {
    ...
    try {
      const resp = await axios.post(`${this.base_url}/${endpoint}`, payload, {
        headers,
        timeout: 25000
      });
      return resp.data;
  ...
  async purchase_product(product_id, quantity = 1, idempotency_key = null, recipient = null) {
    ...
    if (recipient) {
      if (recipient.user_id) {
        payload.user_id = recipient.user_id;
        payload.telegram_id = recipient.user_id;
      }
    ...
    try {
      ...
      logger.info(`purchase_product(product_id=${product_id}, recipient=${JSON.stringify(recipient)}) -> ${resp.status}: ${JSON.stringify(resp.data).slice(0, 300)}`);
      return resp.data;
    } catch (e) {
      if (e.response && e.response.data) {
        return e.response.data;
...
// ===================================================================
// 6. FIRESTORE DATABASE (from firestore.js)
// ===================================================================
class FirestoreDatabase {
  ...
  async ban_user(telegram_id) {
    try {
      ...
      logger.error(`Error in ban_user: ${e.message}`);
    }
  }
  ...
  async unban_user(telegram_id) {
    try {
      ...
      logger.error(`Error in unban_user: ${e.message}`);
    }
  }
  ...
  async set_balance(telegram_id, amount) {
    try {
      ...
      logger.error(`Error in set_balance: ${e.message}`);
    }
  }
  ...
  async update_referral_balance(telegram_id, amount) {
    try {
      ...
      logger.error(`Error in update_referral_balance: ${e.message}`);
    }
  }
  ...
  async increment_order_count(user_id) {
    try {
      ...
      logger.error(`Error in increment_order_count: ${e.message}`);
    }
  }
  ...
  async get_pending_withdrawals() {
    try {
      ...
      snap.forEach(docSnap => {
        const data = docSnap.data();
        if (data && (data.status || "").toUpperCase() === "PENDING") {
  ...
  async approve_withdrawal(withdrawal_id, admin_note = "", admin_id = null) {
    try {
      ...
      if (!w || (w.status || "").toUpperCase() !== "PENDING") {
        return false;
      }
  ...
  async get_referral_list(user_id, limitCount = 20) {
    try {
      ...
      const refs = [];
      snap.forEach(docSnap => {
        refs.push({ id: docSnap.id, ...docSnap.data() });
      });
  ...
  async get_promo_code(code) {
    try {
      ...
      if (snap.exists()) {
        return { code: formattedCode, ...snap.data() };
      }
      return null;
    } catch (e) {
  ...
  async use_promo_code(code, user_id) {
    ...
    if (!promo) {
      return [false, 0.0, "Invalid code."];
    }
    if (parseInt(promo.used_count || 0, 10) >= parseInt(promo.max_uses || 0, 10)) {
      return [false, 0.0, "Code already fully used."];
    }
    if (promo.expires_at && new Date() > new Date(promo.expires_at)) {
      return [false, 0.0, "Code expired."];
    }
    if (parseInt(promo.user_restricted_id || 0, 10) && parseInt(promo.user_restricted_id, 10) !== user_id) {
      return [false, 0.0, "This code is not for you."];
    }
    if (await this.has_user_used_code(code, user_id)) {
      return [false, 0.0, "You have already redeemed this code."];
    }
  ...
  async list_promo_codes() {
    try {
      ...
      const codes = [];
      snap.forEach(docSnap => {
        codes.push({ code: docSnap.id, ...docSnap.data() });
      });
      return codes;
    } catch (e) {
    ...
    try {
      ...
      snap.forEach(docSnap => {
        ...
        if (key === "maintenance_mode") {
          targetSettingsObj.MAINTENANCE_MODE = String(val) === "1" || val === true;
        } else if (key === "withdrawal_fee_percent") {
          targetSettingsObj.WITHDRAWAL_FEE_PERCENT = parseFloat(val);
        } else if (key === "max_deposit_limit") {
          targetSettingsObj.MAX_DEPOSIT_LIMIT = parseFloat(val);
        } else if (key === "max_withdraw_limit") {
          targetSettingsObj.MAX_WITHDRAW_LIMIT = parseFloat(val);
        } else if (key === "max_daily_orders") {
          targetSettingsObj.MAX_DAILY_ORDERS = parseInt(val, 10);
        } else if (key === "report_events") {
          targetSettingsObj.REPORT_EVENTS = String(val) === "1" || val === true;
        } else if (key === "telegram_stars_markup") {
          targetSettingsObj.TELEGRAM_STARS_MARKUP = parseFloat(val);
        } else if (key === "telegram_premium_markup") {
          targetSettingsObj.TELEGRAM_PREMIUM_MARKUP = parseFloat(val);
        }
      });
    } catch (e) {
  ...
  async save_setting(key, value, targetSettingsObj = null) {
    try {
      ...
      if (targetSettingsObj) {
        await this.load_settings(targetSettingsObj);
      }
    } catch (e) {
  ...
  async set_game_markup(game_code, markup) {
    try {
      await setDoc(doc(this.db, "game_config", game_code), {
        markup_percent: parseFloat(markup)
      }, { merge: true });
    } catch (e) {
  ...
  async get_product_price_override(product_id) {
    try {
      ...
      if (snap.exists()) {
        const data = snap.data();
        return data && data.price_override !== undefined && data.price_override !== null
  ...
  async get_dashboard_stats() {
    ...
    try {
      ...
      logger.error(`Error fetching deposits stats: ${e.message}`);
    }

    let total_withdrawals = 0;
    let total_withdrawal_amount = 0.0;
    ...
    try {
      ...
      logger.error(`Error fetching withdrawals stats: ${e.message}`);
    }

    let total_orders = 0;
    let revenue_today = 0.0;
    const today = new Date().toISOString().slice(0, 10);
    try {
      ...
      ordSnap.forEach(d => {
        ...
        if (data.status === "COMPLETED") {
          ...
          if ((data.created_at || "").startsWith(today)) {
            revenue_today += parseFloat(data.charged_price || 0.0);
          }
        }
      });
    } catch (e) {
      logger.error(`Error fetching orders stats: ${e.message}`);
    }
...
// ===================================================================
// 7. CATALOG SERVICE (from catalog.js) – with caching optimizations
// ===================================================================
class CatalogService {
  constructor(api_client, db) {
    ...
    // In-memory caches with longer TTL
    this._catalog_cache = null;
    this._stars_cache = null;
    this._premium_cache = null;
    this._cache_time = 0;
    this._cache_ttl = 600; // 10 minutes
  }

  async _get_telegram_game_code() {
    if (this._telegram_code) {
      return this._telegram_code;
    }
    ...
    if (res && res.success && res.catalogues) {
      ...
      logger.info(`Found Telegram game code: ${exact_code}`);
      return exact_code;
    }
    const games_res = await this.api_client.get_games();
    if (!games_res || !games_res.success) {
      logger.error("Failed to fetch games list from API");
      return null;
    }
    ...
    for (const game of games) {
      ...
      if (name.includes("telegram") || code.toLowerCase().includes("telegram") || name.includes("topup") || code.toLowerCase().includes("topup")) {
        ...
        logger.info(`Found Telegram game code from game list: ${code} (name: ${game.name})`);
        return code;
      }
    }
    logger.error("Could not find any suitable game code for Telegram/topup.");
    return null;
  }
  ...
  async get_telegram_catalogue(forceRefresh = false) {
    ...
    if (!res || !res.success) {
      logger.error(`Failed to fetch catalogue for game ${game_code}: ${JSON.stringify(res)}`);
      return [];
    }
        ...
        if (override !== null && override !== undefined) {
          ...
          if (kind === "stars") {
            ...
            if (parsed && parsed[0] === "stars") {
              ...
              computed = amount * 3; // Stars price = amount × 3 ETB
            } else {
  ...
  async get_telegram_stars_packages(forceRefresh = false) {
    if (!forceRefresh && this._stars_cache) {
  ...
  async get_telegram_premium_plans(forceRefresh = false) {
    if (!forceRefresh && this._premium_cache) {
...
// ===================================================================
// 8. MAIN APPLICATION – with smart editing and photo deletion
// ===================================================================
...
const userLastMessage = new Map(); // userId -> { chatId, messageId, type: 'text'|'photo' }

function getUserSession(userId) {
  if (!userSessions.has(userId)) {
    userSessions.set(userId, { state: null, data: {} });
  }
...
// Helper: send or edit a text message (edits if last message is text, else deletes and sends)
async function sendOrEdit(ctx, text, extra = {}) {
  const userId = ctx.from.id;
  const chatId = ctx.chat.id;
  const prev = userLastMessage.get(userId);

  // If there's a previous text message, edit it
  if (prev && prev.type === 'text') {
    try {
      await ctx.telegram.editMessageText(chatId, prev.messageId, undefined, text, {
        parse_mode: "HTML",
        ...extra
      });
      return { chatId, messageId: prev.messageId }; // same ID
    } catch (e) {
      // If edit fails (e.g., message deleted), fallback to delete and send new
      try {
        await ctx.telegram.deleteMessage(prev.chatId, prev.messageId);
      } catch (_) {}
      userLastMessage.delete(userId);
    }
  } else if (prev) {
    // Previous message is a photo or other media – delete it
    try {
      await ctx.telegram.deleteMessage(prev.chatId, prev.messageId);
    } catch (_) {}
    userLastMessage.delete(userId);
...
// Helper: send or replace a photo (always deletes previous and sends new photo)
...
const ADMIN_CHAT_IDS = Array.isArray(ADMIN_CHAT_ID)
  ? ADMIN_CHAT_ID
  : (typeof ADMIN_CHAT_ID === 'number' ? [ADMIN_CHAT_ID] : []);

const REPORT_CHAT_ID = REPORT_CHANNEL_ID ? REPORT_CHANNEL_ID : PROOF_CHANNEL_ID;
...
async function check_channel_membership(ctx) {
  ...
  const channels = [];
  if (PROOF_CHANNEL_ID) {
    channels.push(["Proof Channel", PROOF_CHANNEL_ID]);
  }
  if (NOTICE_CHANNEL_ID && NOTICE_CHANNEL_ID !== PROOF_CHANNEL_ID) {
    channels.push(["Notice Channel", NOTICE_CHANNEL_ID]);
  }
  if (!channels.length) {
    return true;
  }
  ...
  for (const [name, channel_id] of missing) {
    ...
    try {
      ...
      try {
        ...
        link_text = `Channel ID: ${channel_id}`;
      }
    }
  ...
  if (ctx.callbackQuery) {
    try {
      await ctx.answerCbQuery("Please join the required channels first.", { show_alert: true });
    } catch (_) {}
...
async function report_event(bot, text, parse_mode = "HTML") {
  if (!appSettings.REPORT_EVENTS || !REPORT_CHAT_ID) {
    return;
  }
  try {
...
async function clear_last_photo(ctx, session) {
  // Also clear from userLastMessage if it matches
  const userId = ctx.from.id;
  if (session && session.data && session.data.last_photo) {
    ...
    // Remove from userLastMessage if it's the same message
    const prev = userLastMessage.get(userId);
    if (prev && prev.chatId === chat_id && prev.messageId === message_id) {
      userLastMessage.delete(userId);
    }
  }
}
...
async function register_user_implicit(ctx, db, bot) {
  ...
  if (await db.is_banned(user.id)) {
    if (!ctx.callbackQuery) {
      await sendOrEdit(ctx, `${EMOJI_CROSS} You are banned from using this bot.`);
    }
    return false;
  }
...
async function verify_payment(reference, method = "telebirr") {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      logger.info(`Verification attempt ${attempt + 1} for ref: ${reference}`);
      const resp = await axios.post(url, payload, { headers, timeout: 30000 });
      logger.info(`Verification attempt ${attempt + 1} -> status ${resp.status}`);
      ...
      if (expected_account && receiver_account) {
        if (extract_last4(receiver_account) !== extract_last4(expected_account)) {
          logger.warn(`Receiver account mismatch: ${receiver_account} vs expected ${expected_account}`);
          return null;
        }
      }
      ...
      if (expected_name && receiver_name) {
        if (receiver_name.trim().toUpperCase() !== expected_name.trim().toUpperCase()) {
          logger.warn(`Receiver name mismatch: ${receiver_name} vs expected ${expected_name}`);
          return null;
        }
      }
      ...
      if (payment_date) {
        try {
          ...
          if (!isNaN(txn_dt.getTime())) {
            ...
            if (ageMinutes > MAX_TXN_AGE_MINUTES) {
              logger.info(`Transaction too old: ${ageMinutes.toFixed(1)} min`);
              return null;
            }
          }
        } catch (e) {
          logger.warn(`Could not parse date ${payment_date}: ${e.message}`);
        }
      }
...
async function place_order_flow(ctx, session, db, api_client, bot, payment_method = "wallet", txn_id = "", verified_amount = 0.0) {
  ...
  if (payment_method !== "wallet" && verified_amount > 0) {
  ...
  logger.info("📤 Placing order with:");
  logger.info(`  game_code: ${game_code}`);
  logger.info(`  catalogue_name: ${package_name}`);
  ...
  if (!pid) {
    await sendOrEdit(ctx,
      `${EMOJI_CROSS} Missing recipient. Please re-enter your Telegram username.`,
  ...
  logger.info(`📥 Order response: ${JSON.stringify(order_res)}`);
  ...
  if (order_res && order_res.success) {
    ...
    } else if (order_res.pin) {
      codes_delivered = `🔑 <code>${order_res.pin}</code>`;
    } else if (order_res.code) {
      codes_delivered = `🔑 <code>${order_res.code}</code>`;
    }

    await db.create_order(
      user.id, api_order_id, "COMPLETED",
      game_name, service_name, player_id, nickname,
      package_name, api_price, charged_price,
      markup, JSON.stringify(order_res)
    );
    ...
    await sendOrEdit(ctx,
      `${EMOJI_CROSS} <b>Purchase Failed</b>\n\n⚠️ ${err_msg}`,
      { reply_markup: get_main_inline_keyboard() }
    );
    await report_event(bot,
...
async function main() {
  ...
  bot.command('start', async (ctx) => {
    ...
    // Use the main menu image directly via sendOrEditPhoto to track it
  ...
  bot.command('gencode', async (ctx) => {
    ...
    if (success) {
      await sendOrEdit(ctx, `${EMOJI_SUCCESS} Code <b>${code}</b> created for ${Math.floor(amount)} ETB, uses: ${max_uses}`);
    } else {
      await sendOrEdit(ctx, `${EMOJI_CROSS} Code already exists.`);
    }
  });
  ...
  bot.on('callback_query', async (ctx) => {
    ...
    if (data === "back_to_main") {
      ...
      const text = `${EMOJI_HOME} <b>Main Menu</b>`;
    ...
    if (data === "menu_orders") {
      ...
      let text;
      if (!orders.length) {
        text = `${EMOJI_ORDER} <b>No orders yet.</b>`;
      } else {
    ...
    if (data === "menu_support") {
      ...
      const kb = get_support_keyboard();
      await sendOrEdit(ctx, help_msg, { reply_markup: kb });
      return;
    }
    ...
    if (data === "profile_referral") {
      ...
      const kb = {
        inline_keyboard: [
          [{ text: "Back to Profile", callback_data: "menu_profile", icon_custom_emoji_id: ID_BACK }]
        ]
      };
    ...
    if (data === "profile_redeem") {
      ...
      const kb = {
        inline_keyboard: [[{ text: "Cancel", callback_data: "cancel_action", icon_custom_emoji_id: ID_CANCEL }]]
      };
      await sendOrEdit(ctx, msgText, { reply_markup: kb });
      return;
    }
    ...
    if (data === "menu_service" || data === "back_to_service") {
      ...
      const text = `${EMOJI_GAME} <b>Choose a Telegram service:</b>`;
      await sendOrEdit(ctx, text, { reply_markup: get_service_inline_keyboard() });
      return;
    }
    ...
    if (data.startsWith("svc_telegram_")) {
      ...
      // Show loading message (will be edited later)
      ...
      try {
        ...
        await sendOrEdit(ctx,
          `${EMOJI_CROSS} Failed to load plans.`,
          { reply_markup: { inline_keyboard: [[{ text: "Back", callback_data: "back_to_service", icon_custom_emoji_id: ID_BACK }]] } }
        );
        return;
      }
    ...
    if (data.startsWith("pkg_idx:")) {
      ...
      const cancel_kb = {
        inline_keyboard: [[{ text: "Cancel", callback_data: "cancel_action", icon_custom_emoji_id: ID_CANCEL, style: "danger" }]]
      };
    ...
    if (data.startsWith("pay_method:")) {
      ...
      if (method === "wallet") {
        ...
        const cancel_kb = {
          inline_keyboard: [[{ text: "Cancel", callback_data: "order_cancel", icon_custom_emoji_id: ID_CANCEL, style: "danger" }]]
        };
        await clear_last_photo(ctx, session);
        // Send photo (this will replace any previous text/photo)
    ...
    if (data === "menu_deposit") {
      ...
      const text = `${EMOJI_DEPOSIT} <b>Choose a deposit method</b>\n\nSelect how you'd like to add funds to your balance.`;
      await sendOrEdit(ctx, text, { reply_markup: get_deposit_keyboard() });
      return;
    }
    ...
    if (data === "menu_withdraw") {
      ...
      if (available < MIN_WITHDRAW_BIRR) {
        const text = `${EMOJI_CROSS} Minimum withdrawal is ${MIN_WITHDRAW_BIRR} ETB.\nYour withdrawable balance (non-referral) is ${Math.floor(available)} ETB.`;
        await sendOrEdit(ctx, text, { reply_markup: get_main_inline_keyboard() });
        return;
      }
      ...
      const text = `${EMOJI_TELEBIRR} <b>Enter your Telebirr account number / phone:</b>\n(e.g., 0967197797)`;
      await sendOrEdit(ctx, text, { reply_markup: cancel_kb });
      return;
    }
    ...
    if (data === "withdraw_confirm") {
      ...
      const admin_kb = {
        inline_keyboard: [
          [
            { text: "Approve", callback_data: `admin_approve_wth:${withdrawal_id}`, icon_custom_emoji_id: ID_CONFIRM },
            { text: "Decline", callback_data: `admin_decline_wth:${withdrawal_id}`, icon_custom_emoji_id: ID_CANCEL }
          ]
        ]
      };
      ...
      for (const admin_id of ADMIN_CHAT_IDS) {
        try {
          ...
          logger.error(`Could not send withdrawal notification to admin ${admin_id}: ${e.message}`);
        }
      }
    ...
    if (ADMIN_CHAT_IDS.includes(userId)) {
      ...
      if (data === "admin_deposits") {
        ...
        for (const dep of pending) {
          text += (
            `🆔 <code>${format_deposit_id(dep.id)}</code> | User: ${dep.user_id}\n` +
      ...
      if (data === "admin_withdrawals") {
        ...
        for (const w of pending) {
          text += (
            `🆔 <code>${format_withdrawal_id(w.id)}</code> | User: ${w.user_id}\n` +
      ...
      if (data === "admin_promo_create") {
        ...
        await sendOrEdit(ctx,
          ...
          { reply_markup: { inline_keyboard: [[{ text: "Back", callback_data: "admin_back", icon_custom_emoji_id: ID_BACK }]] } }
        );
        return;
      }
      ...
      if (data === "admin_promo_list") {
        ...
        let text;
        if (!codes.length) {
          text = `${EMOJI_INFO} No active promo codes.`;
        } else {
      ...
      if (data.startsWith("admin_search_id:")) {
        ...
        await sendOrEdit(ctx,
          ...
          { reply_markup: { inline_keyboard: [[{ text: "Back", callback_data: "admin_search_by_id", icon_custom_emoji_id: ID_BACK }]] } }
        );
        return;
      }
      ...
      if (data === "admin_ban") {
        ...
        await sendOrEdit(ctx, `${EMOJI_BAN} <b>Ban User</b>\n\nEnter the user's Telegram ID:`, {
          reply_markup: { inline_keyboard: [[{ text: "Back", callback_data: "admin_user_manage", icon_custom_emoji_id: ID_BACK }]] }
        });
        return;
      }
      ...
      if (data.startsWith("admin_price_type:")) {
        ...
        if (!packages.length) {
          await sendOrEdit(ctx, "No packages found.", {
            reply_markup: { inline_keyboard: [[{ text: "Back", callback_data: "admin_back", icon_custom_emoji_id: ID_BACK }]] }
          });
          return;
        }
      ...
      if (data.startsWith("admin_price_select:")) {
        ...
        await sendOrEdit(ctx, "Enter new price in ETB for this product (or 0 to remove override):", {
          reply_markup: { inline_keyboard: [[{ text: "Cancel", callback_data: "admin_back", icon_custom_emoji_id: ID_BACK }]] }
        });
        return;
      }
      ...
      if (data === "admin_close") {
        clearUserSession(userId);
        // Try to delete the admin message (we can't easily get its ID, so we send a new message that will replace any previous)
  ...
  bot.on('message', async (ctx) => {
    ...
    // Admin Reply to decline message
    ...
    if (session.state === STATE_ADMIN_SEARCH_BY_ID) {
      ...
      if (search_type === "order") {
        ...
        if (!order) {
          order = await db.get_order_by_id(text);
        }
        if (order) {
          ...
          result_text = `${EMOJI_CROSS} Order not found.`;
        }
      } else if (search_type === "deposit") {
        let deposit = null;
        if (/^\d+$/.test(text)) {
          deposit = await db.get_deposit_by_id(text);
        }
        if (deposit) {
          ...
          result_text = `${EMOJI_CROSS} Deposit not found.`;
        }
        ...
        if (!withdrawal && text.toUpperCase().startsWith("EX")) {
          ...
          if (parsed) {
            withdrawal = await db.get_withdrawal_by_id(parsed);
          }
        }
        if (!withdrawal && /^\d+$/.test(text)) {
          withdrawal = await db.get_withdrawal_by_id(`WTH-${text}`);
        }
        if (withdrawal && Object.keys(withdrawal).length) {
          ...
          result_text = `${EMOJI_CROSS} Withdrawal not found.`;
        }
      }
      session.state = STATE_ADMIN_MAIN;
    ...
    if (session.state === STATE_ENTER_UID) {
      ...
      if (resolved_name) {
        session.data.nickname = resolved_name;
      }
    ...
    if (session.state === STATE_PAYMENT_TXN_ID) {
      ...
      if (result) {
        ...
        if (attempts >= 3) {
          ...
          clearUserSession(userId);
          await sendOrEdit(ctx,
            `${EMOJI_CROSS} Verification failed after 3 attempts. Please try again later or contact support.`,
            { reply_markup: get_main_inline_keyboard() }
          );
        } else {
          await sendOrEdit(ctx,
            `${EMOJI_CROSS} Not found. ${3 - attempts} tries left.\nRe‑enter the reference.`,
            { reply_markup: { inline_keyboard: [[{ text: "Cancel", callback_data: "order_cancel", icon_custom_emoji_id: ID_CANCEL }]] } }
          );
        }
        return;
      }
