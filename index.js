...
// (with parse_amount M/B, bulk overrides, cache clearing)
// ===================================================================
...
// ===================================================================
// 3. UTILITY FUNCTIONS (with M/B support)
// ===================================================================
...
// ===================================================================
// 7. CATALOG SERVICE – with bulk overrides and cache clearing
// ===================================================================
class CatalogService {
  ...
  async get_telegram_catalogue(forceRefresh = false) {
    ...
    // Bulk fetch all overrides
    const overridesMap = new Map();
    const allOverrides = await this.db.get_all_product_overrides();
    for (const ov of allOverrides) {
      if (ov.price_override !== undefined && ov.price_override !== null) {
        overridesMap.set(ov.id, ov.price_override);
      }
    }
    ...
    for (const it of items) {
      ...
      // Check override map
      const override = overridesMap.get(product_id);
      if (override !== undefined && override !== null) {
        newItem._override_price = override;
      } else {
        // Compute price normally
        if (kind === "stars") {
...
async function main() {
  ...
  bot.on('callback_query', async (ctx) => {
    ...
    if (ADMIN_CHAT_IDS.includes(userId)) {
      ...
      if (data === "admin_stars_markup") {
        ...
        // Clear catalog cache so new markup applies immediately
  ...
  // Webhook endpoint – raw body needed for Telegraf
  app.use(WEBHOOK_PATH, express.raw({ type: 'application/json' }));

  // Telegraf webhook handler. Do NOT mount it under WEBHOOK_PATH,
  // otherwise Express strips the path before Telegraf sees it.
